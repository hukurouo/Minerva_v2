...
def eval_jockey
  ...
  jockey_index = CSV.table("intermediate/jockey/jockey_index_2020.csv", {:encoding => 'UTF-8', :converters => nil})
  jockey_index.each do |jockey|
    jockey_map = {}
    (2017..2019).each do |year|
      ...
      (1..10).each do |i|
        ...
        race_result.each do |result|
          if result[:jockeyid] == jockey[:jockeyid]
            id = result[:id]
            ...
            rank = result[:rank]
            unless jockey_map[race_type]
              jockey_map[race_type] = [0,0,0,0]
              jockey_map[race_type] = add_result(jockey_map[race_type],rank)
            else
              jockey_map[race_type] = add_result(jockey_map[race_type],rank)
            end
          end
    ...
    CSV.open("intermediate/jockey/2017-2019/#{jockey[:jockeyid]}.csv", "w") do |csv| 
