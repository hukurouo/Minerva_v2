...
def today()
  ...
  race_index = CSV.table("today/#{@dir_name}/#{@file_name}/index.csv", {:encoding => 'UTF-8', :converters => nil})
  ...
  race_ids.each_with_index do |id, index|
    ...
    write(result, @year, @place)
    p id
  end
end

def write(race_results, year, place)
  CSV.open("today/#{@dir_name}/#{@file_name}/datas.csv", "a") do |csv| 
